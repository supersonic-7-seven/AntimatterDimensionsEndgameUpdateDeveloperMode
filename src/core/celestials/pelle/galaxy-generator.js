...
export const GalaxyGenerator = {
  ...
  get gainPerSecondPostCap() {
    if (!Pelle.hasGalaxyGenerator) return 0;
    return new Decimal(Math.max(1, Math.pow(this.galGenInstability, Math.log10(Math.max(Math.pow(player.celestials.pelle.galaxyGenerator.generatedGalaxies / 1e12, 0.9), 1))))
    ).toNumber();
  },

...
export const infinityUpgrades = {
  ...
  unspentIPMult: {
    ...
    effect: () => Decimal.max(Currency.infinityPoints.value.dividedBy(2), DC.E1E15).pow(1.5).plus(1),
    formatEffect: value => formatX(value, 2, 2),
    charged: {
      description: "Multiplier to 1st Antimatter Dimension based on unspent Infinity Points, powered by Teresa level",
      effect: () => Decimal.max(Currency.infinityPoints.value.dividedBy(2), DC.E1E15).pow(Math.sqrt(Ra.pets.teresa.level) * 1.5).plus(1),
      formatEffect: value => formatX(value, 2, 2)
    }
  },
  dimboostMult: {
